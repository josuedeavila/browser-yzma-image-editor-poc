...
#   make build-wasm       # compile wasm/main.go -> web/vendor/yzma/yzma-vlm.wasm
...
CLONE_DIR := /tmp/browser-yzma-image-editor-poc-src
...
# CLI to download the matching prebuilt llama.cpp WASM libraries (the
# multimodal/mtmd support this PoC needs is already in that same build, no
# extra flag needed). TinyGo cannot build the C++ of llama.cpp itself, so
...
# does not matter next to the ~280MB model+projector download.
.PHONY: build-wasm
build-wasm:
	GOOS=js GOARCH=wasm go build -o $(VENDOR_DIR)/yzma-vlm.wasm ./wasm
...
build-wasm-tinygo:
	tinygo build -target wasm -o $(VENDOR_DIR)/yzma-vlm.wasm ./wasm
...
# timing (?mode=cpu on that port) against :8090's multi-threaded one — see
